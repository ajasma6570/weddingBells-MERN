...
import { useGetActiveCateringMutation } from '../../Redux/Admin/adminApiSlice';
import { toastError } from '../toast'


export default function AdminActiveCatering() {
  ...
  const [getActiveCatering] = useGetActiveCateringMutation()
  ...
  useEffect(()=>{
    const fetchData = async() => {
      setLoading(true); 
      const res = await getActiveCatering()
        if(res.data.status === 200){
...
<div className='block bg-transparent m-4 p-2 overflow-x-auto shadow-stone-600 shadow-lg h-screen'>
    <h1 className='text-xl font-semibold text-gray-800'>Active Catering</h1>
    <table>
        <thead>
